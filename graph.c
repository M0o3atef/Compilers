...
void exNode
    (nodeType * p,
        ...
        switch (p->type) {
    	case typeStringCon:
            sprintf(word, "c(\"%s\")", p->con.sValue);
	        break;
	    case typeIntCon:
            sprintf(word, "c(%d)", p->con.iValue);
	        break;
	    case typeFloatCon:
            sprintf(word, "c(%f)", p->con.fValue);
	        break;
        case typeId:
            sprintf(word, "id(%s)", sym[p->id.i]->name);
            break;
        case typeOpr:
            switch (p->opr.oper) {
            case WHILE:
        ...
        graphBox(s, & w, & h);
        cbar = c; * ce = c + w; * cm = c + w / 2;
        /* node is leaf */
        if (p->type == typeStringCon || p->type == typeIntCon || p->type == typeFloatCon || p->type == typeId || p->opr.nops == 0) {
        ...
        /* total node width */
        if (w < che - c) {
            cbar += (che - c - w) / 2; * ce = che; * cm = (c + che) / 2;
        }
        /* draw node */
        graphDrawBox(s, cbar, l);
...
void graphBox(char * s, int * w, int * h) { * w = strlen(s) + del; * h = 1;
}
void graphDrawBox(char * s, int c, int l) {
    int i;
